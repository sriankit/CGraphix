...
void giveOutput(float out[20][2])
{
    ...
    for(i=0;i<NEdges-1;i+=1)
        ...
        //drawLine(out[i][0],out[i][1],out[i+1][0],out[i+1][1]);
    LineDDA(out[NEdges-1][0],out[NEdges-1][1],out[0][0],out[0][1]);
    //drawLine(out[NEdges-1][0],out[NEdges-1][1],out[0][0],out[0][1]);
    //glFlush();
    //glutSwapBuffers();
}
...
void TransformPoints(void)
{
    ...
    for (k=0;k<NEdges;k++)
    scanf("%f%f",&intial[k][0],&intial[k][1]);
    ...
    switch(choice)
    {
        ...
        for(k=0;k<NEdges;k++)
        {
            theMatrix[0][0]=intial[k][0];
            theMatrix[1][0]=intial[k][1];
            theMatrix[2][0]=1;

            Translate(TransDistX,TransDistY);

            final[k][0]=theMatrix[0][0];
            final[k][1]=theMatrix[1][0];
        }
        break;
        ...
        case 3:     printf("Enter Scaling ratios along X & Y\n=>");
